...
$(document).ready(function() {
		...
		$('#myImg').bind('mouseup', function() {
			...
			if ((imgH / imgW) <= 1) {
				if (imgW <= 200) {
					...
					$('#popup_title').css('left', imgW + 315);
					$('#popup_id').css('left', imgW + 580);
					$('#popup_body').css('left', imgW + 315);
					$('#popup_reply').css('left', imgW + 315);
					$('#popup_vote').css('left', imgW + 315);
				} else if (imgW <= 300) {
					$('#img1').css('width', imgW + 200);
					$('#popup_title').css('left', imgW + 215);
					$('#popup_id').css('left', imgW + 480);
					$('#popup_body').css('left', imgW + 215);
					$('#popup_reply').css('left', imgW + 215);
					$('#popup_vote').css('left', imgW + 215);
				} else if (imgW <= 400) {
					$('#img1').css('width', imgW + 100);
					$('#popup_title').css('left', imgW + 115);
					$('#popup_id').css('left', imgW + 380);
					$('#popup_body').css('left', imgW + 115);
					$('#popup_reply').css('left', imgW + 115);
					$('#popup_vote').css('left', imgW + 115);
				} else if (imgW <= 600) {
					$('#img1').css('width', imgW);
					$('#popup_title').css('left', imgW + 15);
					$('#popup_id').css('left', imgW + 280);
					$('#popup_body').css('left', imgW + 15);
					$('#popup_reply').css('left', imgW + 15);
					$('#popup_vote').css('left', imgW + 15);
				} else if(imgW > 600){
					$('#img1').css('width', '600px');
					$('#popup_title').css('left', '615px');
					$('#popup_id').css('left', '880px');
					$('#popup_body').css('left', '615px');
					$('#popup_reply').css('left', '615px');
					$('#popup_vote').css('left', '615px');
				}
			} else {
				if (imgW <= 200) {
					$('#img1').css('width', imgW + 300);
	...
	$('#btn_idCheck').click(function() {
		$.ajax({
			type : "POST",
			url : "idCheck.do",
			data : $('#join_userid').serialize(), // 회원가입폼의 id 입력창 안에 있는
													// value값을 불러온다
...
function joinField() {
	if (document.join_form.join_userid.value.length < 6) {
		...
		return;
	} else if ((document.join_form.phone1.value == "")
...
function informModify() {
	if (document.informModify_form.userpw.value < 6) {
		...
		return;
	} else if ((document.informModify_form.phone1.value == "")
			|| (document.informModify_form.phone2.value == "")
			|| (document.informModify_form.phone3.value == "")) {
		alert("핸드폰번호를 입력해주세요.");
	...
	if (document.informModify_form.userpw.length < 6) {
		alert("비밀번호를 6~10자로 입력해주세요.");
		document.informModify_form.userpw.focus();
		return;
	}
